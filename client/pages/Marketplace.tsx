...
import { listListings, Listing, ListingCategory, ListingCondition } from '@/lib/marketplace';
...
export default function MarketplacePage() {
  ...
  const load = async () => {
    const res = await listListings({ q: q || undefined, category: category === 'all' ? undefined : category, condition: condition === 'all' ? undefined : condition });
    setItems(res);
  };
  ...
  return (
    <div className="container mx-auto p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link to="/sell-item"><Button>Sell an Item</Button></Link>
      </div>
          ...
          <Link to={`/listing/${it.id}`} key={it.id} className="block">
            <Card className="overflow-hidden hover:shadow-lg transition">
              ...
              <CardContent className="p-4 space-y-1">
                ...
                <div className="text-sm text-gray-400">{it.category} • {it.condition}</div>
                <div className="text-green-400 font-bold">₹{it.price}</div>
              </CardContent>
            </Card>
          </Link>
        ))}
        {items.length === 0 && <div className="text-gray-400">No listings found.</div>}
      </div>
    </div>
  );
}
